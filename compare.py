...
obs_indices = np.arange(0, npts, 10)  # observe every 10th variable
h = np.zeros((len(obs_indices), npts))
for i, idx in enumerate(obs_indices):
    h[i, idx] = 1
from scipy.spatial.distance import cdist
length_scale = 20  # or another empirically chosen value
covlocal = np.exp(-cdist(np.arange(npts).reshape(-1, 1), np.arange(npts).reshape(-1, 1), 'euclidean') / length_scale)
